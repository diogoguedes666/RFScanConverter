...
def convert_csv_content(csv_file):
    ...
    for index, row in df.iterrows():
        ...
        # Format the frequency and value
        if '.' in frequency or ',' in frequency:
            freq_parts = frequency.split('.') if '.' in frequency else frequency.split(',')
            formatted_frequency = "{:3}.{:<6}".format(freq_parts[0][:3], freq_parts[1][:6].ljust(6, '0'))
        else:
            formatted_frequency = frequency.ljust(10)
        
        converted_data.append("{}, {}".format(formatted_frequency, value))
